...
// That suits lexing that jumps around a file — to an offset named by an index,
// past a payload, back to a span already scanned — since such jumps
...
// window — a reused cursor repointed at different underlying data — which the
// window cannot notice on its own: it keys what it holds by reader identity,
// and that identity did not change.
func (w *Window) Drop() {
	w.n = 0
	w.end = math.MaxInt64
	w.err = nil
